...
if doCal, Scal.calgood=0; lastcaldate=date;
	...
	for p=1:10, %drawTarg(Scal.Cloc(1:9,:),5,Sexp.col.BLU,0); %target grid
		...
		if p==1, sampmat=nan(0);
			...
			while ~KbCheck, %until key is pressed
				sampmat=[pollPHstream(1:Sexp.nPHsamp); sampmat]; end
		...
		%ALL OTHER TARGETS	
			...
			while and(~KbCheck,all(varmult.*nanvar(sampmat(end-Scal.Nsamp+1:end,1:end-1),1)<=2*Scal.vars)), %if any var increases beyond 1.25*Scal.vars
				sampnow=pollPHstream(1:Sexp.nPHsamp);
                disp([max(nanvar(sampmat(end-Scal.Nsamp+1:end,1:end-1),1)) 2*Scal.vars])
			...
			while and(~KbCheck,or(isnan(sampmat(end,1)),any(2*varmult.*nanvar(sampmat(:,1:end-1),1)>Scal.vars))),
	...
	[Scal]=initStream(Pcell,sampmove,Scal); end %THIS ONE ADDS THE Z-DIM to the XY, defining Scal.Clist as the triple of xyz-columns in the raw data stream
save C:\Users\VMILlab\Desktop\polhemusCalData.mat Scal lastcaldate P Pcell; end
%lastssid=Sexp.Sinit;
